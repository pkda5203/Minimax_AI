...
//        //this.exit_function( 0 );
	...
	{
     //   System.out.println("New GameBoard was Created!!\n");
	    this.currentTurn = 1;
	}
    ...
    {
        int currScr =this.getScore(player);
        int eval = currScr-12;  //considering the fact that optimal score to draw is 12 for both
        return eval;
    }
    ...
    public int get2Score( int player )
    {
        //reset the scores
        int playerScore = 0;
...
//CHECK for 3-in-a-row
    public int get3Score( int player )
    {
        //reset the scores
        int playerScore = 0;
...
public boolean playPiece2( int column ) {
...
//void findSucessor(GameBoard currentGame)
//{
//int y = currentGame.getCurrentTurn();
//GameBoard temp=new GameBoard( "dsfsdf" );
//for( int k = 0; k < 6; k++ )
//{
//for( int j = 0; j < 7; j++ )
//{
//temp.playBoard[k][j]=currentGame.playBoard[k][j];
//}
//}
//
//for (int i=0; i<7; i++)
...
//}
    ...
    public void copyFunc (GameBoard one1, GameBoard two2)
    {
        int y = two2.getCurrentTurn();
        for( int k = 0; k < 6; k++ )
        {
            for( int j = 0; j < 7; j++ )
            {
                one1.playBoard[k][j]=two2.playBoard[k][j];
            }
        }
        one1.currentTurn=y;
        one1.pieceCount=two2.pieceCount;
    }

    public void ChangeCurrentTurn(GameBoard currGame)
    {
        if (currGame.currentTurn==1)
        {
            currGame.currentTurn=2;
        }
        if (currGame.currentTurn==2)
        {
            currGame.currentTurn=1;
        }
    }
